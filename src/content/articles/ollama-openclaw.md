...
> | 我的狀況 | 建議路線 |
> |---------|----------|
> | 🐣 **什麼都沒裝，完全白紙** | 照 Step 1 → 2 → 3 依序走，約 10 分鐘就能開始對話 |
> | ✅ **已裝 Ollama，想接 OpenClaw** | 直接跳 [Step 2：安裝 OpenClaw](#step-2安裝-openclaw-並啟動) |
...
```
![ollama launch openclaw 官網頁面](/images/articles/ollama-openclaw/ollama-launch-openclaw.png)
...
安裝完成後，畫面會出現模型選擇清單。選「**Cloud**」即可，不需要下載模型到本機。
...
macOS 可能跳出安全性提示，點擊「允許」即可。（Node.js 是 OpenClaw 的實際執行環境）
...
出現就緒畫面，代表 OpenClaw 已成功啟動！

> <img src="/images/dock_head_s.png" alt="鴨編" width="24" style="vertical-align: middle;"> **社群回報雲端模型效果不錯**——可以直接幫你設定 Telegram、Email 等功能。如果 AI 說「請你自己裝」，可以跟它耐心溝通：「請你幫我裝，我不會」，通常它會配合。
...
恭喜 🎉 你的龍蝦正式啟動！接下來可以繼續聊天，或往下看 Ollama 免費額度說明，以及進階設定方式。

---

## Ollama 免費額度說明

Ollama 的雲端模型有免費額度可用（[官方說明](https://ollama.com/pricing)）。想查看你目前的用量：
...
額度用完了怎麼辦？可以改用其他[雲端 API（如 Gemini Flash）](/articles/gemini-api-setup)，或參考文末說明在本機下載開源模型。

---

## 進階：手動設定 config（可選）

> 用 `ollama launch openclaw` 已經可以開始對話。  
> 如果你想**手動控制模型選擇、混用本機與雲端，或整合既有 API Key**，才需要這個步驟。
...
如果回傳一串 JSON（包含模型清單），代表 Ollama 運行正常。

> 🚨 **如果連不到？**
> - macOS / Windows：確認 Ollama app 有在運行（系統工具列應該看得到圖示）
> - Linux：執行 `ollama serve` 啟動服務

### 設定 OpenClaw config.yaml

在 OpenClaw 的設定檔 `config.yaml` 中加入 Ollama：
...
> 💡 如果只想進設定頁調整，不立即啟動，可以用 `ollama launch openclaw --config`。

### 混用既有雲端 API

如果你之前已設定過雲端 API，可以保留它們，把 Ollama 當成額外選項：
...
> 額度用光、想要完全離線，或只是好奇開源模型？這部分說明怎麼把模型下載到你的電腦上。  
...
| 使用場景 | 推薦方案 | 原因 |
|---|---|---|
| 學習、實驗 | Ollama 雲端（免費額度）| 零設定，立即可用 |
| 日常輕度使用 | Ollama + Qwen 7B（本機）| 免費，速度可接受 |
