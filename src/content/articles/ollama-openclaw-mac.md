...
> `<img src="/images/dock_head_s.png" alt="鴨編" width="24" style="vertical-align: middle;">` **常見打錯**：中間的 `launch` 不能少。如果你直接輸入 `ollama openclaw`，會跳這個錯：
>
> ```
> Error: unknown command "openclaw" for "ollama"
> ```
>
> 加上 `launch` 再執行一次就對了。
...
第一次執行會自動安裝，詢問是否用 npm 安裝時按 Enter 確認。終端機會出現這個提示：
...
選 `Yes`（按 Enter），就會開始 `Installing OpenClaw...`：

![終端機執行 ollama launch openclaw，跳出 OpenClaw is not installed. Install with npm? 安裝提示](/images/articles/ollama-openclaw/ollama-launch-terminal.png)

![終端機執行 OpenClaw 安裝指令過程，顯示 Installing OpenClaw 與 npm 安裝輸出](/images/articles/ollama-openclaw/install-openclaw-terminal.png)
...
指令打成了 `ollama openclaw`——少了 `launch`。正確指令是 `ollama launch openclaw`（中間一定要有 `launch`）。這跟 Hermes 的 `unknown integration: hermes` 是同一類問題：Ollama 不認得你打的子指令。加上 `launch` 重打一次即可。
