...
- [ ] [大功能] **藍鴨導入 LINE 會員系統**（Joseph 2026-07-10 提，建議開新 session 專做）。現況：藍鴨已有 Supabase Auth 會員（`member_profiles` + Google OAuth + events/registrations + admin 角色 + Resend 通知）。**複用指標（記憶池已解，非重造）**：① 網站接 LINE 登入走 Supabase Custom OIDC `custom:line`（`589411/lovestrings` 分支 `feat/line-login-remove-lovable`：一行 signInWithOAuth + trigger 補「LINE 用戶無 email」fallback + issuer access.line.me + email_optional）；② LIFF 認人/多頁（`line-booking-system`、`new-cpc`）；③ LINE 推播管道（skill `line-daily-push`）。**待決設計**：(a) LINE Login 取代 Google 還是並存多一選項（建議並存漸進）；(b) 通知走 LINE push 補充/取代 Resend email（LINE 會員可能無 email→push 更合適）；(c) 報名流程是否改「LINE Login→原生報名→LINE 確認」。**關鍵關聯**：這可能就是 external_url 退場所指的「正式模式」最終形態——三件事（LINE 會員 / 8/26 正式報名 / external_url 退場）是同一決策
