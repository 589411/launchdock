...
每次新增文章、頁面，或大改版之後，**push 前確認**：
1. `npm run build` 本地測試 build 無誤
2. `git push` → Cloudflare 自動部署並更新 sitemap

### Google Search Console

- Sitemap 提交網址：`https://launchdock.app/sitemap-index.xml`（**只需提交一次**）
- Google 會定期自動爬取，不需要每次 push 後手動更新
- 若新文章需要加速被索引：Search Console → URL Inspection → 手動申請建立索引

### 相關檔案

| 檔案 | 說明 |
|------|------|
| `public/robots.txt` | 允許爬蟲並指向 sitemap |
| `astro.config.mjs` | sitemap 設定（過濾 /admin/、/auth/ 路徑）|
| `dist/sitemap-index.xml` | build 輸出（勿手動修改）|
