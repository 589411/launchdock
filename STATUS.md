...
**✅ 2026-08-10 已執行：`cli-guide` 標題已改（中英雙版，commit `33943e7`）**
`CLI 是什麼？10 分鐘上手命令列，跟終端機／Shell 差在哪（附 18 個常用指令）`
——承諾都有內文依據（18 個 unique 指令／內文「基本操作 10 分鐘就能上手」）；刻意用「上手」不用
「看完」，因為 `estimatedMinutes: 12`，喊「10 分鐘看得完」會跟站上顯示的閱讀時間打對台。
**➡️ 下一個動作（SEO 線）＝ ~08-24 單看 `cli-guide` 這一頁的 CTR**（0.7% → 目標 3%，約 +90 點擊／28 天）。
比對時只看這一頁、只看 CTR，不要again被站台平均值干擾。
...
**同張報表「頁面會重新導向」113 筆＝105 筆正常殘影 ＋ 8 筆真 bug（已修）**（2026-08-17 查證，
見 `docs/seo-indexing-fix.md` §六，含全部七桶的「要不要按驗證修正」對照表）。
- ✅ **已修並上線**（commit `142ac22`）：七月修正**漏了帶 query 的連結**——`articlesPath()` 回傳無斜線，
  被每篇文章頁的麵包屑用；首頁 SceneCard 也硬編 `/articles?scene=`。⇒ 每爬一篇文章就重新發現一次無斜線網址。
  修後 dist 無斜線 `?scene=` 連結 157 → 0，**全站任何形式的無斜線內部連結 0**。
- 其餘 105 筆是 7/18 之前的舊網址殘影，**已驗證是封閉集合**（7/18 後新增的 10 個 slug 命中 0 筆）。
...
**FAQ／schema 已判定（2026-08-10，Joseph 手動確認）**：GSC「搜尋外觀」**完全無資料**——
零豐富結果曝光。但**錯不在標記**：同日驗證線上首頁 JSON-LD，`@type=FAQPage`、`@context` 正確、
8 題全部有 `Question.name` + `acceptedAnswer.text`，`home-faq.ts` 那套生成是好的。
最可能是 **Google 自 2023 年起只給「政府／醫療權威網站」FAQ 版位**（請以 Google Search Central
現行文件為準）⇒ 若成立，這是預期行為、**不是待修 bug**。
➡️ **決策：停止投資 FAQ rich result**，把 FAQ 的價值改押「給 AI 搜尋／LLM 讀」那條線
（與站上既有 `llms.txt` + 全站 `.md` 端點同方向）。選配確認：用 GSC**網址審查**查首頁，
若「已偵測到的項目」列出「常見問題」＝Google 解析成功只是不給版位，政策說成立。
（首頁 CTR 12.9% 是品牌字帶的——`launchdock` 38.5%、`藍鴨` 13.1%——**不能歸因 FAQ**。）
...
**2026-09-19 新文章 `mcp-servers-picks`（MCP Server 精選＋安全鐵律）中英雙版已 gate 通過並 push（`83e3bf9`）** ⬅️ 最新
...
- ✅ **Joseph gate 通過（2026-09-19）**，commit `83e3bf9` 已 push；**首頁 FAQ 已加第 9 則**「MCP Server 該裝哪些？隨便裝會有風險嗎？」
  （`home-faq.ts`，畫面與 FAQPage schema 皆驗過 9 則）。
- ⭐ **下一個具體動作**：補兩張真實 UI 截圖（Claude 設定 → Connectors 清單／Notion OAuth 授權頁）→
  `./scripts/add-image.sh mcp-servers-picks ~/Desktop/*.png` → **插圖後必跑機敏掃描**（左下角帳號區、email）→ commit。
- 順帶：`home-faq.ts` 第 5 則「系統提示詞」仍連 `/meetup/`（7/29 工作坊已過），可改連 `set-system-prompt` 文章——**支線，未動**。
...
- ✅ **已 commit + push + 線上驗收**（Joseph 2026-09-02 當場 gate 通過）：commit `5374cdc`（文章）／
  `7cb4846`（英文 tooltip + parser bug）／`593ca88`（延伸閱讀 i18n），Cloudflare Pages 已部署。
  線上實測：中英文章頁 200、三張 SVG 200、英文頁 tooltip 全英文、英文延伸閱讀連 `/en/articles/`、
  `prompt-engineering` 已自動連到 `set-system-prompt`（parser 修好的證據）。
  <details><summary>當時預留的 commit 指令（已執行，保留供對帳）</summary>
  `git add src/content/articles/git-guide.md src/content/articles/en/git-guide.md public/images/articles/git-guide/ src/data/concepts.yaml docs/article-registry.json BACKLOG.md STATUS.md && git commit && git push`</details>
- ⭐ **下一個具體動作（唯一待辦）**：~2026-10-01 看 GSC 有沒有「git 教學／git 是什麼／版本控制」長尾進來（站上第一篇吃 Git 字的文章）。
...
- ✅ **2026-09-02 第三批（Joseph 指示「要做」）——英文 tooltip**：
  ① `concepts.yaml` 60 個概念全部補 `shortDescEn`，插件在英文文章優先用它（沒填才退回中文）。
  ② 修 tooltip 寬度：定位父層是很窄的 inline `<a>`，`max-width` 對它無效 → 加 `width: max-content`（中英都變好看）。
  ③ 🔴 **順手揪出既有真 bug：YAML parser 的鍵名 regex `^([A-Za-z]...` 只認英文字母開頭**，
     導致 `系統提示詞`／`分享連結不等於私密連結`／`語音輸入補上下文` 三個概念被靜默吃掉，
