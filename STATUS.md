...
## 下一個具體動作 ⭐
**2026-07-29 藍鴨組合器會員登入＋提示詞儲存（M1–M4 已寫，等 Joseph 瀏覽器實測）**——devplan（`prompt-builder-auth-devplan.md`）原假設導 Firebase；第 0 步盤點發現**本站早有完整 Supabase Google 登入會員系統**（AuthButton/AuthProvider/`member_profiles` 16 人/AdminMembers），故**改用 Supabase、不導 Firebase**（Joseph 拍板）。分支 `feat/prompt-builder`，兩個 commit：① **M1** migration `010_create_saved_prompts`（owner-only RLS + server-side 20 則/人上限 trigger + REVOKE EXECUTE 硬化）**已套 production `lxudxtpfenotkpgmhomq` 並功能實測**（插 20 筆 OK、第 21 筆被 check_violation 擋、rolled-back 零殘留、advisor 無新 WARN）；② **M2+M3+M4** 把 `launchdock-meet/prompt-builder.html` 移植成 `/tools/prompt-builder/`（`PromptBuilder.tsx` React island + Astro 頁），用站上主題 token（明暗雙主題，優於原 dark-only），prose 輸出**驗證與原版 byte 一致**；工具本體人人可用（試＋複製），只有「儲存/我的提示詞（載回編輯/重新命名/刪除）」促登入。build 綠（150 頁）。**下一步（需 Joseph）**：① 瀏覽器實測登入→儲存→重整仍在→跨使用者擋讀（M5 實測，需 Supabase env + Google 帳號）；② 決定開放問題 Q2（「更多職業卡」是否設會員限定當誘因）、Q3（登入是否順帶勾選電子報）；③ 加站上入口（Header 工具下拉／`set-system-prompt` 文章導流）；④ 決定是否 push 分支 + 英文版工具（內容需翻 PAIN/ROLE/PREF/TUNE，已記 BACKLOG）。M6 會員名單＝既有 `member_profiles`，等同已完成。
