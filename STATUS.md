...
## 進度脈絡（新的在上）
- 2026-07-19 生態系導流閉環：首頁 `index.astro` 加「學完，拿去用」入口區塊（Lab／Masters 兩張卡，導向 lab./masters.launchdock.app），已 build 綠 + 瀏覽器實測 + push main（CF Pages 自動 build）。同日兩個子網域站也各加「← 回藍鴨主站」連結（masters-hub、launchdock-lab 兩 repo，已 push）。→ 主站↔子站雙向流量閉環完成。可選下一步：Header/Footer 也加常駐入口。
